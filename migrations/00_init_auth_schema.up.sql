...
comment on table {{ index .Options "Namespace" }}.users is 'Back: Stores user login data within a secure schema.';

-- back.refresh_tokens definition
...
comment on table {{ index .Options "Namespace" }}.refresh_tokens is 'Back: Store of tokens used to refresh JWT tokens once they expire.';

-- back.instances definition
...
comment on table {{ index .Options "Namespace" }}.instances is 'Back: Manages users across multiple sites.';

-- back.audit_log_entries definition
...
comment on table {{ index .Options "Namespace" }}.audit_log_entries is 'Back: Audit trail for user actions.';

-- back.schema_migrations definition

CREATE TABLE IF NOT EXISTS {{ index .Options "Namespace" }}.schema_migrations (
	"version" varchar(255) NOT NULL,
	CONSTRAINT schema_migrations_pkey PRIMARY KEY ("version")
);
comment on table {{ index .Options "Namespace" }}.schema_migrations is 'Back: Manages updates to the auth system.';
