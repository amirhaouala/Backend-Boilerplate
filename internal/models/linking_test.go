...
func (ts *AccountLinkingTestSuite) TestCreateAccountDecisionWithAccounts() {
	userA, err := NewUser("test", "", "test@example.com", "", "authenticated", nil)
	require.NoError(ts.T(), err)
	...
	userB, err := NewUser("test", "", "test@samltest.id", "", "authenticated", nil)
	require.NoError(ts.T(), err)
...
func (ts *AccountLinkingTestSuite) TestMultipleAccounts() {
	...
	userB, err := NewUser("test", "", "test-b@example.com", "", "authenticated", nil)
	require.NoError(ts.T(), err)
	require.NoError(ts.T(), ts.db.Create(userB))
