...
func (ts *ResendTestSuite) TestResendSuccess() {
	// Create user
	u, err := models.NewUser("foo", "123456789", "foo@example.com", "password", ts.Config.JWT.Aud, nil)
	...
	emailUser, err := models.NewUser("bar", "", "bar@example.com", "password", ts.Config.JWT.Aud, nil)
	require.NoError(ts.T(), err, "Error creating test user model")
