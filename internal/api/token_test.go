...
func (ts *TokenTestSuite) SetupTest() {
	...
	u, err := models.NewUser("test", "", "test@example.com", "password", ts.Config.JWT.Aud, nil)
...
func (ts *TokenTestSuite) createBannedUser() *models.User {
	u, err := models.NewUser("banned", "", "banned@example.com", "password", ts.Config.JWT.Aud, nil)
	require.NoError(ts.T(), err, "Error creating test user model")
	t := time.Now()
	u.EmailConfirmedAt = &t
...
func (ts *TokenTestSuite) TestAllowSelectAuthenticationMethods() {
	...
	companyUser, err := models.NewUser("test", "12345678", "test@company.com", "password", ts.Config.JWT.Aud, nil)
	t := time.Now()
